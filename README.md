...
- `nfa.py` 단독 실행 + Thompson e-NFA 구성요소/전이표 출력 지원
- `dfa.py` 단독 실행 + 입력 regex로 e-NFA 및 Reduced DFA 생성/출력 지원
...
또한 AST size, arc upper bound, exact arc count를 함께 출력한다.
...
직접 실행 시 regex 입력을 받아 e-NFA를 생성하고,
동치 Reduced DFA를 만들어 각 구성요소와 상태 전이 정보를 출력한다.
...
- 콘솔에서 정규표현식 입력
- AST nested list 출력
- AST size, arc upper bound, exact arc count 출력
- `ast_tree.html` 생성
- 기본 브라우저 자동 오픈 시도
...
- 콘솔에서 정규표현식 입력
- AST nested list 출력
- e-NFA 구성요소(states, alphabet, start/final states) 출력
- NFA transition table 출력
